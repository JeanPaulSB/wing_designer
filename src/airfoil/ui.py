...
class Window(QtWidgets.QMainWindow):
    ...
    def validateInputs(self):
        ...
        if all([nacaisValid,chordisValid,pointsisValid]):
            self.widget.airfoil = Naca(naca.text(),float(chord.text()),int(points.text()))
            self.widget.plot()
    ...
    def getFile(self):
        if self.widget.airfoil:
            ...
            filename = QFileDialog.getSaveFileName(self,"Save file",f"NACA{self.widget.airfoil.digits}.csv",options = options)
